%SaveFragLetterSloanImages
%code to generate fragmented letters, similar to the VOSP test, then save as .tif files
%here using Sloan letters, loaded from image files
%specify image size, a set of IntactProp levels (proportion of the letter that is intact where lower=more fragmented) levels,
% plus checksize under image parameters below
%
...
LetterFileDir = '/Users/john.greenwood/Documents/MATLAB/Stimuli/Letters/Sloan/'; %folder where the letter images are - change as needed!

%set up folder to save images to
filetofind = 'SaveFragLetterSloanImages.m';
ThisDir=which(filetofind); %find this file
ThisDir=ThisDir(1:end-length(filetofind)); %get the directory where files are (cutting off filename and matlab folder)
SaveDir = strcat(ThisDir,'Images/'); %where images will be saved

%% image parameters

NumSets         = 3; %how many sets of letters do we want to generate?
SetNames        = [3 4 5]; %avoids over-writing old sets if only want to generate one set at a time but with a higher number than previous runs

IntactProp      = [1.58 2.82 5.01 8.91 15.85 28.18 50.12 89.13 100]./100;%proportion intact - where 0=fully fragmented and 1 is fully intact
LogSen          = log10(1./(IntactProp)); %formula to convert fragmentation into sensitivity (NB use proportions not percentages)
NumIntactLevels = numel(IntactProp);

%TestChars = {'H','O','T','V'}; %4AFC
%TestChars = {'H','O','T','V','U','X'}; %6AFC
%TestChars = {'C','D','H','K','N','O','R','S','V','Z'};%10AFC
TestChars = {'C','D','E','F','H','K','N','P','R','U','V','Z'}; %12AFC - matched to visual acuity testing in UK Biobank
...
CheckSize    = 11;%2;%20; %each 'check' in the noise image (that becomes the 'fragments') is X pixels wide - with 500 pixel width, stroke width = 100pix so 20pix checks gives 5/stroke
...
for ss=1:NumSets
    disp(strcat('Letter set number: ',num2str(SetNames(ss))));
    for ii=1:NumIntactLevels
        ...
        ContrastInc = ((3*(IntactProp(ii))))-(1.5); %0; %where ~ -1.5 is full black and ~ +1.5 is full white - used to alter balance of white vs black pixels in noise image - trying to start at a level close to intactprop rather than simply at 0

        for ll=1:NumLetters
            Tol             = 0.0005; %how much variation we're comfortable with in the difference between desired and actual IntactProp (prev 0.001)
            ...
            while KeepGoing
                if IntactProp(ii)==1 %for whole letters just make a black image to be quick about it
                ...
                if abs(IntactProp(ii)-ActualIntactProp)<Tol %if actual value is close enough to desired value
                    KeepGoing = 0; %then stop
                else %keep going but alter ContrastInc
                    if (ActualIntactProp-IntactProp(ii))>0 %then need more fragmentation (IntactProp needs to be lower)
                ...
                if cnt>100000 %code taking too long %prev 1000
                    Tol = Tol*1.5; %increase tolerance to avoid code freezing up %prev *2
                    cnt=0; %reset counter
                end
            end
        ...
        disp(' ');
        disp(strcat('Desired Proportion Intact:',num2str(IntactProp(ii)))); disp(' ');
        ...
        %% save the images
        for ll=1:NumLetters
            fName = sprintf('%sSet%1d-%2.2fIntact-%s.png',SaveDir,SetNames(ss),IntactProp(ii)*100,TestChars{ll});
